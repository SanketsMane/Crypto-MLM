  ...
  return (
    <Card className={`h-full ${className ?? ''}`}>
      ...
      <div className="px-5 pb-5">
        ...
        <div className="mt-3.5 space-y-2">
          <div className="flex items-center gap-2 rounded-[5px] border border-line bg-canvas px-3 py-2.5">
            ...
            <button onClick={() => profile && copy('link', profile.referralLink)} aria-label="Copy referral link"
                    className="shrink-0 rounded-[3px] p-1.5 text-ink-2 transition hover:bg-card hover:text-ink">
          ...
          <div className="flex gap-2">
            <button onClick={() => profile && copy('code', profile.userCode)}
                    className="flex min-w-0 flex-1 items-center justify-center gap-1.5 rounded-[5px] border border-line px-3 py-2.5 text-[12.5px] font-medium text-ink-2 transition hover:border-ink-3 hover:text-ink">
        ...
        <dl className="mt-4 grid grid-cols-3 gap-px overflow-hidden rounded-[5px] border border-line bg-line">
          {[
